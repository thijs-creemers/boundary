...
[source,bash]
----
source .env          # boundary new already generated this with a real JWT_SECRET
clojure -M:repl
----

NOTE: `boundary new` writes a `.env` (git-ignored) containing a generated `JWT_SECRET` — no need to export one by hand. `source .env` (or `direnv allow`) loads it into the shell.

`clojure -M:repl` starts a headless nREPL server on port 7888. Connect your editor
(or the Boundary MCP server) to it, then eval:
