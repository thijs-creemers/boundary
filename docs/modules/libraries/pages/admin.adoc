...
2. Add `+#include "conf/dev/admin/{module}.edn"+` to the `#merge` vector in `config.edn`
3. `(ig-repl/reset)` to reload configuration

For Aero-based config loading, the `#include` path is resolved from the classpath root. Use `conf/dev/admin/...`, not `admin/...`.
