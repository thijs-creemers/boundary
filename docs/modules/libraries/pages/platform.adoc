...
Tokens are emitted with no per-handler wiring: the shared page layout renders
`<meta name="csrf-token">`, and a global `htmx:configRequest` listener attaches the
`X-CSRF-Token` header to every HTMX request. Plain (non-HTMX) forms include a hidden
field:
