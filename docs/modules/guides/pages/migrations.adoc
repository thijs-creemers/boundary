...
Boundary uses https://github.com/yogthos/migratus[Migratus] for SQL-based, versioned migrations. This guide covers conventions, safety patterns, and common recipes.
...
| `libs/{lib}/resources/boundary/{lib}/migrations/`
...
The migration system (`boundary.platform.shell.database.migrations`) discovers all manifests and merges their directories at startup.
...
. **Per-tenant schema** — tenant-specific tables. The tenant module applies these automatically during provisioning.
...
For very large tables (millions of rows), write a Clojure migration script instead of SQL and run it as a one-off task outside the migration framework.
