...
This guide covers deployment patterns beyond Docker (which is documented in xref:deployment.adoc[] and `dev-docs/DOCKER_DEVELOPMENT.adoc`). Reference configs are in `resources/deploy/`.
...
All deployments need these variables. See `resources/deploy/systemd/boundary.env.example` for a complete template.

[cols="1,1,2",options="header"]
|===
| Variable | Required | Description

| `BND_ENV` | Yes | `production` (or `acc` for staging)
...
The unit file automatically runs migrations via `ExecStartPre` before starting the server. On failure it restarts after 10 seconds (max 5 retries per minute).
...
- HTTP -> HTTPS redirect
- WebSocket upgrade support for `/ws` (realtime module)
...
`/health/ready` returns 503 when any dependency is unreachable, making it safe for load balancer health checks — traffic won't be routed until the database is connected.
