...
Boundary's module-centric architecture organizes code by domain. This guide shows how to manually create a new module (e.g., `billing`, `inventory`) with the proper structure.
...
(defrecord BillingService [invoice-repo]
  ...
  (create-invoice [_ invoice-data]
    ...
    ;; - Logs "create-invoice" entry with correlation ID and tenant context
