...
TIP: **Use the Scaffolder**: The recommended approach is to use Boundary's module scaffolder, which generates production-ready modules with zero errors in seconds. See xref:reference/scaffolder.adoc[Module Scaffolder Reference] for details.

[[quick-start]]
== Quick Start with Scaffolder

Generate a complete module with a single command:
...
See xref:reference/scaffolder.adoc[Module Scaffolder Reference] for complete documentation.
...
Create `src/boundary/billing/schema.clj`:
...
Create `src/boundary/billing/ports.clj`:
...
Create `src/boundary/billing/core/billing.clj`:
...
Create `src/boundary/billing/shell/service.clj`:
...
Create `src/boundary/billing/shell/persistence.clj`:
...
Create `src/boundary/billing/shell/http.clj`:
...
Create `src/boundary/billing/shell/cli.clj`:
...
In `src/boundary/shell/interfaces/http/server.clj` or similar:
...
(defn create-app-routes
  [config services]
  (concat
    (user-http/user-routes (:user-service services))
    (billing-http/billing-routes (:billing-service services))))
----
