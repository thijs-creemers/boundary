...
(defprotocol IUserRepository
  ...
  (find-user-by-email [this email tenant-id]
    "Retrieve user by email within a tenant.
    
    Args:
      email - Email address
      tenant-id - UUID of tenant for multi-tenancy
...
(defrecord DatabaseUserRepository [db-ctx]
  ...
  (find-user-by-email [_ email tenant-id]
    (let [query {:select [:*]
                ...
                :where [:and
                       [:= :email email]
                       [:= :tenant_id (type-conv/uuid->string tenant-id)]
                       [:is :deleted_at nil]]}
          result (db/execute-one! db-ctx query)]
      (when result
        (db-record->user-entity result))))
...
(defn db-record->user-entity
  ...
  {:id (type-conv/string->uuid (:id record))
   :email (:email record)
   :name (:name record)
   :tenant-id (type-conv/string->uuid (:tenant_id record))
...
(defn user-entity->db-record
  ...
  {:id (type-conv/uuid->string (:id entity))
   :email (:email entity)
   :name (:name entity)
   :tenant_id (type-conv/uuid->string (:tenant-id entity))
...
* xref:functional-core-imperative-shell.adoc[Functional Core / Imperative Shell]
* xref:modules-and-ownership.adoc[Modules and Domain Ownership]
