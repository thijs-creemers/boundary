...
* ✅ **Structured context management** with correlation IDs and tenant isolation
...
- **Automatic Context Propagation**: Correlation IDs, tenant context, and tracing flow seamlessly
...
(defprotocol ILoggingContext
  ...
  (with-tenant-context [this context tenant-id])
...
{:correlation-id "req-12345"     ; Request correlation ID
 ...
 :tenant-id      "tenant-abc"    ; Multi-tenant context
...
(defn with-tenant-id [context tenant-id])
