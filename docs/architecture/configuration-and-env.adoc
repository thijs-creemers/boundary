...
Elara uses a layered configuration approach with clear precedence rules:
...
{:profile :dev
 :database {:host #env DATABASE_HOST
           ...
           :name #env [DATABASE_NAME "elara_dev"]
...
(ns elara.system.wiring
  (:require [elara.config :as config]
            [elara.shell.adapters.database :as db-adapter]
            [elara.shell.adapters.http :as http-adapter]
            [elara.shell.interfaces.http.routes :as routes]
            [clojure.tools.logging :as log]))
...
(defn api-routes
  ...
  (let [routes []]
    (-> routes
        ...
        (conj (elara.modules.user.http/routes))
        
        ;; Conditional module inclusion
        (with-module-routes :billing features elara.modules.billing.http/routes)
        (with-module-routes :workflow features elara.modules.workflow.http/routes)
...
|`DATABASE_NAME` |String |Database name |`elara_{profile}`
