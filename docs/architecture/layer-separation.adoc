...
[abstract]
--
This document establishes explicit rules and guidelines for maintaining the Functional Core / Imperative Shell architectural boundaries in {project-name}. It provides concrete examples of correct and incorrect implementations, dependency constraints, and testing strategies that ensure architectural integrity.
--
...
**Prerequisites:** Understanding of {xref-overview}[Architecture Overview] and functional programming principles
...
(ns elara.core.user
  (:require [elara.ports :as ports]))  ; Only abstract interfaces

;; ✅ CORRECT: Shell depends on core and implements adapters
(ns elara.shell.user
  (:require [elara.core.user :as user-core]  ; Core business logic
            [elara.ports :as ports]          ; Abstract interfaces
            [elara.shell.db :as db]))        ; Concrete adapters

;; ❌ WRONG: Core depending on shell
(ns elara.core.user
  (:require [elara.shell.db :as db]))        ; ❌ Core → Shell dependency

;; ❌ WRONG: Core depending on concrete adapters
(ns elara.core.user
  (:require [elara.adapters.postgres :as pg])) ; ❌ Core → Adapter dependency
----
...
(ns elara.shell.user-test
  (:require [clojure.test :refer [deftest is testing use-fixtures]]
            [elara.shell.user :as user-shell]
            [elara.shell.db :as db]
            [elara.test-utils :as test-utils]))
...
1. **{xref-components}[Component Architecture]** - How to organize namespaces and modules
2. **{xref-ports-adapters}[Ports and Adapters]** - Implementing dependency inversion
3. **{xref-data-flow}[Data Flow Architecture]** - Request processing patterns
