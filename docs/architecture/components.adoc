...
* ✅ **Namespace organization** and how code is structured across layers
* ✅ **Component responsibilities** and their specific roles in the architecture
* ✅ **Interaction patterns** between different types of components
* ✅ **System wiring** and dependency management strategies
* ✅ **Module boundaries** and how to maintain architectural integrity
...
Elara's component architecture follows a strict layered approach where each layer has specific responsibilities and clear boundaries. The diagram above shows the main components and their interactions.

== Namespace Structure and Organization

=== Adopted Module-Centric Architecture

✅ **Elara implements a module-centric architecture where each domain module owns its complete functionality stack:**
...
│   │   │   ├── server.clj
│   │   │   ├── middleware.clj
│   │   │   ├── common.clj      # Shared HTTP utilities
│   │   │   └── routes.clj      # Aggregates module routes
│   │   ├── cli/
│   │   │   ├── main.clj        # CLI entry point
│   │   │   ├── parsing.clj     # Common CLI utilities
...
[source,clojure]
----
;; USER MODULE - All user-related code together
(ns elara.user.ports)
...
(defn calculate-user-benefits [user-data] ...)

(ns elara.user.shell.service)
(defn register-user [system request] ...)

;; BILLING MODULE - Separate concerns
...
(def Payment [:map [:amount [:double {:min 0.01}]] [:currency string?]])

(ns elara.billing.core.pricing)
(defn calculate-total-cost [line-items tax-rate] ...)
----
...
(ns elara.billing.core.invoicing
  (:require [elara.user.ports :as user-ports]    ; ✓ Depend on user ports
            [elara.billing.core.pricing :as pricing]))
...
* ✅ Domain-focused naming and concepts
* ✅ Dependencies only on other core components and ports
...
* ✅ Clojure protocols defining capabilities
* ✅ Business-domain focused method names
* ✅ Technology-agnostic interfaces
* ✅ Owned by functional core layer
...
* ✅ Coordinate between core functions and adapters
* ✅ Handle input validation and output formatting
* ✅ Manage transactions and error handling
* ✅ Execute effects returned by core functions
* ✅ No business logic - pure orchestration

[source,clojure]
----
;; Example: Shell Service Component
(ns elara.shell.services.user-service
  "User service - orchestrates user operations between interfaces and core."
  (:require [elara.core.user :as user-core]
            [elara.ports :as ports]
            [elara.schema :as schema]
            [malli.core :as m]
            [malli.error :as me]
...
* ✅ Concrete implementations of port protocols
* ✅ Handle technology-specific concerns (SQL, HTTP, etc.)
* ✅ Transform between domain objects and external formats
...
* ✅ Transform service results to protocol format
* ✅ Handle protocol-specific concerns (status codes, headers, etc.)
* ✅ No business logic - pure interface handling

[source,clojure]
----
;; Example: HTTP Interface Component
(ns elara.shell.interfaces.http.handlers
...
|**Interface → Service** |✅ |HTTP handlers call service functions |`user-service/register-user`
|**Service → Core** |✅ |Services call pure core functions |`user-core/process-registration`
|**Service → Adapter** |✅ |Services use adapters via ports |`ports/create-user`
|**Core → Port** |✅ |Core depends on abstract interfaces |`ports/find-user-by-email`
|**Adapter → Port** |✅ |Adapters implement port protocols |`defrecord ... IUserRepository`
|**Core → Adapter** |❌ |Core never depends on concrete adapters |Direct database calls
|**Core → Service** |❌ |Core never calls shell services |Calling service functions
|**Adapter → Core** |❌ |Adapters never call core functions |Business logic in adapters
|===
...
(ns elara.shell.system.wiring
  ...
  (:require [elara.shell.adapters.database.postgresql :as pg-adapter]
            [elara.shell.adapters.notifications.smtp :as smtp-adapter]
            [elara.shell.services.user-service :as user-service]
            [elara.shell.interfaces.http.server :as http-server]
...
(defn- create-system-services [config]
  ...
  (reify elara.ports/ISystemServices
    (current-timestamp [_]
...
(ns elara.shell.main
  ...
  (:require [elara.shell.system.wiring :as wiring]
            [clojure.tools.logging :as log])
  (:gen-class))
...
(defn start-application
  ...
  [profile]
  (log/info "Starting Elara application" {:profile profile})
...
(defn stop-application
  ...
  []
  (log/info "Stopping Elara application")
...
(ns elara.core.user-test
  (:require [clojure.test :refer [deftest is testing]]
            [elara.core.user :as user-core]))
...
(ns elara.shell.services.user-service-test
  (:require [clojure.test :refer [deftest is testing]]
            [elara.shell.services.user-service :as user-service]
            [elara.test.mocks :as mocks]))
...
(ns elara.shell.adapters.database.postgresql-test
  (:require [clojure.test :refer [deftest is testing use-fixtures]]
            [elara.shell.adapters.database.postgresql :as pg-adapter]
            [elara.test.database :as test-db]))
...
1. **{xref-data-flow}[Data Flow Architecture]** - How data moves through components
2. **{xref-integration-patterns}[Integration Patterns]** - Interface-specific patterns
3. **{xref-configuration}[Configuration and Environment Management]** - Component wiring
