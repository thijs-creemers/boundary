...
✅ **Boundary implements a module-centric architecture where each domain module owns its complete functionality stack:**
...
(ns boundary.billing.core.invoicing
  (:require [boundary.user.ports :as user-ports]    ; ✓ Depend on user ports
...
(ns boundary.shell.interfaces.http.handlers
  "HTTP request handlers - translate between HTTP and service layer."
  (:require [boundary.shell.services.user-service :as user-service]
...
(ns boundary.shell.system.wiring
  ...
  (:require [boundary.shell.adapters.database.postgresql :as pg-adapter]
            [boundary.shell.adapters.notifications.smtp :as smtp-adapter]
            [boundary.shell.services.user-service :as user-service]
            [boundary.shell.interfaces.http.server :as http-server]
...
(ns boundary.shell.main
  "Application entry point and lifecycle management."
  (:require [boundary.shell.system.wiring :as wiring]
            [clojure.tools.logging :as log])
  (:gen-class))
...
(ns boundary.shell.services.user-service-test
  (:require [clojure.test :refer [deftest is testing]]
            [boundary.shell.services.user-service :as user-service]
...
(ns boundary.shell.adapters.database.postgresql-test
  (:require [clojure.test :refer [deftest is testing use-fixtures]]
            [boundary.shell.adapters.database.postgresql :as pg-adapter]
