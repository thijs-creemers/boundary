...
[abstract]
--
This document explains the protocol-based database adapter architecture implemented in {project-name}. Completed October 24, 2025, this refactoring provides a unified, consistent interface for working with multiple database systems (SQLite, PostgreSQL, H2, MySQL) through well-defined protocols and database-specific implementations.
--
...
----
boundary.shell.adapters.database.{database-name}/
...
(ns boundary.shell.adapters.database.postgresql.core
  (:require [boundary.shell.adapters.database.protocols :as proto]
            [boundary.shell.adapters.database.postgresql.connection :as conn]
            [boundary.shell.adapters.database.postgresql.query :as query]
            [boundary.shell.adapters.database.postgresql.metadata :as meta]
...
(ns boundary.shell.adapters.database.factory
  (:require [boundary.shell.adapters.database.postgresql.core :as pg]
            [boundary.shell.adapters.database.sqlite.core :as sqlite]
            [boundary.shell.adapters.database.h2.core :as h2]
            [boundary.shell.adapters.database.mysql.core :as mysql]))
...
(require '[boundary.shell.adapters.database.factory :as dbf]
         '[boundary.shell.adapters.database.protocols :as proto])
...
For detailed usage examples, see {xref-multi-db}[Multi-Database Usage Guide].
