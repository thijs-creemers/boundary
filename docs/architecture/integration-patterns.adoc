...
**Prerequisites:** Understanding of {xref-data-flow}[Data Flow Architecture] and {xref-components}[Component Architecture]
...
Elara provides three primary integration interfaces, each tailored to specific use cases while maintaining consistent underlying behavior:
...
Elara uses Ring and Reitit for HTTP handling with a clean separation between routing configuration and handler implementation.
...
(ns elara.shell.interfaces.http.routes
  ...
  (:require [elara.core.user.http :as user-http]
            [elara.core.membership.http :as membership-http]
            [elara.core.billing.http :as billing-http]
            [elara.shell.interfaces.http.middleware :as middleware]
            [elara.shell.interfaces.http.monitoring :as monitoring]
            [reitit.ring :as ring]
...
(ns elara.shell.interfaces.http.common
...
(defn create-problem-details
  ...
  [service-result correlation-id]
  {:type (str "https://api.elara.example/errors/" (name (:error-type service-result)))
...
(ns elara.core.user.http
  "User module HTTP interface - handlers, routes, and schemas."
  (:require [elara.core.user.user :as user-core]
            [elara.core.user.schema :as user-schema]
            [elara.shell.interfaces.http.common :as http-common]
...
(ns elara.core.membership.http
  "Membership module HTTP interface."
  (:require [elara.core.membership.membership :as membership-core]
            [elara.core.membership.schema :as membership-schema]
            [elara.shell.interfaces.http.common :as http-common]))
...
(ns elara.core.billing.http
  "Billing module HTTP interface."
  (:require [elara.core.billing.billing :as billing-core]
            [elara.core.billing.schema :as billing-schema]
            [elara.shell.interfaces.http.common :as http-common]
            [elara.config :as config]))
...
```
src/elara/shell/interfaces/http/
...
(ns elara.shell.interfaces.http.versioning
...
(defn version-aware-user-creation
  ...
  (case (:api-version request)
    :v1 (elara.user.shell.service/register-user-v1 system user-data)
    :v2 (elara.user.shell.service/register-user-v2 system user-data)
    ;; Default to latest stable
    (elara.user.shell.service/register-user-v1 system user-data)))
...
image::{diagrams-dir}/cli-command-lifecycle.png[CLI Command Lifecycle, 800]
...
(ns elara.shell.interfaces.cli.commands
...
(defn user-create-command
  ...
  (let [{:keys [options arguments errors summary]} (parse-opts args user-create-options)
    ...
    (cond
      (:help options)
      {:exit-code 0
       :output (format "Usage: elara user create [OPTIONS]\n\n%s\n\nExamples:\n  elara user create --email john@example.com --name \"John Doe\"\n  elara user create --email jane@example.com --name \"Jane Smith\" --theme dark --notifications" summary)}
...
(defn user-get-command
  ...
  (let [{:keys [options arguments errors summary]} (parse-opts args global-options)
    ...
    (cond
      (:help options)
      {:exit-code 0
       :output (format "Usage: elara user get USER_ID [OPTIONS]\n\n%s\n\nExamples:\n  elara user get 123e4567-e89b-12d3-a456-426614174000\n  elara user get 123e4567-e89b-12d3-a456-426614174000 --json" summary)}
...
(defn user-benefits-command
  ...
  (let [{:keys [options arguments errors summary]} (parse-opts args user-benefits-options)
    ...
    (cond
      (:help options)
      {:exit-code 0
       :output (format "Usage: elara user benefits USER_ID [OPTIONS]\n\n%s\n\nExamples:\n  elara user benefits 123e4567-e89b-12d3-a456-426614174000\n  elara user benefits 123e4567-e89b-12d3-a456-426614174000 --refresh --json" summary)}
      ...
      (not (first arguments))
      {:exit-code 2
       :output "Error: USER_ID is required\n\nUsage: elara user benefits USER_ID"}
...
(ns elara.shell.interfaces.websocket.handlers
  "WebSocket handlers for real-time web interface communication."
  (:require [elara.shell.services.user-service :as user-service]
            [cheshire.core :as json]
            [clojure.tools.logging :as log]))
...
(ns elara.shell.services.integration
...
(ns elara.shell.validation.unified
  "Unified validation logic for all interfaces."
  (:require [malli.core :as m]
            [malli.error :as me]
            [elara.schema :as schema]))
...
[cols="2,2,3,2"]
|===
|Interface |Version Method |Implementation |Backward Compatibility

|**REST API** |URL path versioning |`/api/v1/`, `/api/v2/` |Maintain v1 for 12 months
|**CLI** |Command versioning |`elara v1 user create`, `elara v2 user create` |Auto-migration warnings
|**WebSocket** |Message versioning |`{"version": "v1", "type": "user.get"}` |Client negotiation
|**Schemas** |Schema versioning |Separate schema namespaces |Progressive enhancement
|===
...
[source,clojure]
----
;; API migration and deprecation support
(ns elara.shell.interfaces.migration
  "Support for API migrations and backward compatibility."
  (:require [clojure.tools.logging :as log]))

(defn check-api-deprecation
  "Check if API version is deprecated and add appropriate warnings."
  [interface-type version]
  (let [deprecation-info (get-deprecation-info interface-type version)]
    (when (:deprecated? deprecation-info)
      (log/warn "Deprecated API version used" 
               {:interface interface-type
                :version version
                :sunset-date (:sunset-date deprecation-info)
                :migration-guide (:migration-guide deprecation-info)})
      deprecation-info)))

(defn get-deprecation-info
  "Get deprecation information for API version."
  [interface-type version]
  (case [interface-type version]
    [:rest :v1] {:deprecated? true
                 :sunset-date "2024-12-31"
                 :successor-version :v2
                 :migration-guide "https://docs.elara.com/migration/v1-to-v2"}
    
    [:cli :v1] {:deprecated? false}  ; Current stable
    
    ;; Default - not deprecated
    {:deprecated? false}))

(defn add-migration-headers
  "Add migration-related headers to HTTP responses."
  [response interface-type version]
  (when-let [deprecation (check-api-deprecation interface-type version)]
    (when (:deprecated? deprecation)
      (-> response
          (assoc-in [:headers "Deprecation"] "true")
          (assoc-in [:headers "Sunset"] (:sunset-date deprecation))
          (assoc-in [:headers "Link"] 
                   (str "</api/" (name (:successor-version deprecation)) ">; rel=\"successor-version\""))))))
----
...
(ns elara.shell.interfaces.caching
...
1. **{xref-configuration}[Configuration and Environment Management]** - Environment-specific integration setup
2. **{xref-observability}[Error Handling and Observability]** - Monitoring and debugging across interfaces
