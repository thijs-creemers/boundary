...
(ns boundary.shell.interfaces.http.routes
  ...
  (:require [boundary.core.user.http :as user-http]
            ...
            [boundary.shell.interfaces.http.monitoring :as monitoring]
            [reitit.ring :as ring]
...
```
src/boundary/shell/interfaces/http/
...
(ns boundary.shell.interfaces.http.versioning
...
(ns boundary.shell.interfaces.cli.commands
...
(ns boundary.shell.interfaces.websocket.handlers
...
(ns boundary.shell.services.integration
...
(ns boundary.shell.validation.unified
...
|**CLI** |Command versioning |`boundary v1 user create`, `boundary v2 user create` |Auto-migration warnings
...
(ns boundary.shell.interfaces.migration
...
(defn get-deprecation-info
  ...
  (case [interface-type version]
    [:rest :v1] {:deprecated? true
                 ...
                 :migration-guide "link:{xref-data-flow}[Data Flow Architecture]"}  ; Updated guide reference
...
(ns boundary.shell.interfaces.caching
