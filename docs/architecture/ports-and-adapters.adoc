...
**Ports are abstract interfaces** that define the capabilities needed by the functional core without specifying how those capabilities are implemented. In Elara, ports are defined as Clojure protocols.
...
Elara uses a simple dependency injection pattern where adapters are created and passed to core functions through the shell layer.

[source,clojure]
----
;; System configuration and wiring
(ns elara.shell.system
  (:require [elara.shell.db :as db]
            [elara.shell.email :as email]
...
(ns elara.test.contracts.user-repository-test
  (:require [clojure.test :refer [deftest testing is]]
            [elara.ports :as ports]
            [elara.shell.db :as db]
            [elara.test.adapters :as test-adapters]))
...
(ns elara.shell.adapters.caching
  (:require [elara.ports :as ports]
