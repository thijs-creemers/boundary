...
==== Purpose
Creates a new user account with role-based permissions and tenant association. This endpoint demonstrates the complete validation pipeline, core function integration, and error handling patterns of the Elara framework.
...
|tenant-id |UUID |No |Override default tenant context (admin only)
|===
...
* **Core Function**: `elara.core.user/create-new-user` - Pure function that validates business rules and returns user data with effects
...
* **Business Rules**: 
  ** Email must be unique across tenant
...
[source,bash]
----
elara users create --email="john.doe@example.com" --role=user --active=true
----
...
*Test Coverage:*
* **Unit Tests**: `elara.core.user-test/create-new-user-test` covers business logic
* **Integration Tests**: `elara.shell.http-test/create-user-integration-test` covers full request lifecycle
