...
The Boundary Framework provides a comprehensive Docker-based development environment with intelligent port management, service orchestration, and development tools integration.
...
- **Port conflict resolution**: Uses exact port or fails in Docker (predictable)
...
### `docs` Profile  
**Services**: Application + Database + Documentation Server  
...
- **Predictable behavior**: No surprise port changes in containerized environment  
...
2. **Expose JVM debug port** in `docker-compose.override.yml`:
   ```yaml
   services:
     app:
       ports:
...
1. **Dependencies first**: `deps.edn` copied before source code
2. **Separate build stage**: Dependencies downloaded once
3. **Minimal runtime image**: Only production dependencies in final image
...
3. Select `boundary-dev-tools` container
4. Install Clojure extensions in container

### IntelliJ IDEA
1. Configure **Docker** plugin
2. Set up **Remote JVM Debug** to `localhost:5005`
3. Configure **Clojure REPL** to connect to container

### Emacs/Vim
1. Use **TRAMP** to edit files in containers
2. Configure **nREPL** connection to dev-tools container
3. Set up port forwarding for REPL access
...
While this setup is optimized for development, production deployment considerations:
...
- [Port Management System](../src/boundary/shell/utils/port_manager.clj)
- [Documentation Serving](./DOCUMENTATION_SERVING.md)
