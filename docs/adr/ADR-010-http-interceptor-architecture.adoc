...
4. Provides observability services via `:system` config
...
Applied to all routes via router config:

```clojure
(def config
  {:system {:logger ... :metrics-emitter ... :error-reporter ...}
   :global-interceptors ['boundary.platform.http/logging
                         'boundary.platform.http/metrics
                         'boundary.platform.http/error-reporting]})
...
Global interceptors run first, then route-specific interceptors:
...
4. **Requires System Services**
   - Routes with interceptors need `:system` in config
   - Observability services must be available
   - More complex setup than plain Ring handlers
