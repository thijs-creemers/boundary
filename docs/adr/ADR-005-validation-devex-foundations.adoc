...
- [x] Tenant ID immutability tests
...
3. **Required Field Validation**
   ...
   - Comprehensive required field coverage (email, name, role, tenant-id)
...
5. **Tenant ID Immutability Tests**
   - Business rule enforcement: Cannot change tenant-id after creation
   - Error code verification: `:user.tenant-id/forbidden`
   - Change detection: `calculate-user-changes` function validation
   - Proper error message: "Cannot change tenant-id after user creation"
...
2. **Business Rule Validation Logic** (`src/boundary/user/core/user.clj`)
   ...
   - Impact: Tenant-id and email immutability rules now correctly enforced
   - Changed from: `(not= (:tenant-id changes :from) (:tenant-id changes :to))`
   - Changed to: `(not= (get-in changes [:tenant-id :from]) (get-in changes [:tenant-id :to]))`
...
- ✅ Business rules: Tenant-id immutability, email immutability
...
- ✅ Tenant-id change always fails validation (100 cases)
...
- ✅ Soft deletion preserves identity fields (id, email, tenant-id) (100 cases)
