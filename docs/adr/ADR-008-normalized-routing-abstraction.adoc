...
┌─────────────────────────────────────────────────────────────┐
│                    Modules (Pure Data)                      │
│  user/shell/http.clj, billing/shell/http.clj, etc.          │
...
(defmethod ig/init-key :boundary/http-routes
  [_ {:keys [user-routes billing-routes]}]
  ;; Aggregate all module routes
  (concat user-routes billing-routes))
...
1. Migrate remaining modules (billing, workflow, inventory)
...
- [ ] Migrate `billing` module routes
- [ ] Migrate `workflow` module routes
