...
weight: 50
description: "Observability module: providers, protocols, and configuration"
---
...
*** link:#_modern_approach_multi_layer_interceptor_pattern[Modern Approach: Multi-Layer Interceptor Pattern ⭐]
...
|===
|__ a|
*🎯 Multi-Layer Interceptor Pattern (Recommended Approach)*
...
*Key Benefits:* - ✅ *Automatic Integration*: No manual observability calls required - ✅ *Business Logic Preservation*: Core functions remain pure and focused - ✅ *Consistent Patterns*: Standardized observability across all layers - ✅ *Framework Evolution*: 31/31 methods converted in latest implementation
...
These examples are representative – exact timestamps, thread names and context keys depend on your runtime configuration and log call.
...
* link:#architecture:error-handling-observability[Error Handling & Observability Architecture]
* link:#architecture:observability-integration[Observability Integration Guide]
