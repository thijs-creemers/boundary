---
title: "Validation Developer Guide"
weight: 70
description: "Developer guide for Malli-based validation in Boundary"
---
...
*Functional Core (Pure):* - `boundary.core.validation.messages` - Message templating and suggestion engine - `boundary.core.validation.context` - Contextual message rendering - `boundary.core.validation.codes` - Error code catalog - `boundary.core.validation.result` - Result format utilities - `boundary.core.validation.registry` - Rule registry for coverage tracking

*Imperative Shell (I/O):* - `boundary.core.validation` - Backward-compatible API - Feature flag reading (`BND++_++DEVEX++_++VALIDATION`) - Module-specific service integration
...
|`true`, `1`, `yes`, `on` (case-insensitive) |✅ Yes |Enhanced features active
|`false`, `0`, `no`, `off`, or unset |❌ No |Legacy behavior (default)
|===
...
|`++{++entity}` |Entity type being operated on |`"user"`, `"file"`, `"job"`
...
[source,highlightjs,highlight]
----
(require '[boundary.core.validation.context :as ctx])
...
(require '[boundary.core.validation.context :as ctx])
(require '[boundary.core.validation.codes :as codes])
...
(with-redefs [boundary.core.validation.result/devex-validation-enabled? (constantly false)]
...
(with-redefs [boundary.core.validation.result/devex-validation-enabled? (constantly true)]
  (v/validate-with-transform schema/CreateUserRequest invalid-user nil))
;; => {:valid? false
;;     :errors [{:field :email
;;               :code :invalid-format
...
*DON’T:* - Read `BND++_++DEVEX++_++VALIDATION` directly in core functions - Mix I/O operations with validation logic - Assume specific error message formatting in core - Break existing function signatures without providing legacy support
...
*DON’T:* - Change existing error codes without deprecation process - Use generic codes when domain-specific codes exist - Create duplicate codes for the same validation
...
*DON’T:* - Use technical jargon in user-facing messages - Blame users ("You provided invalid data") - Use humor or sarcasm in error messages - Write messages longer than necessary
...
*DON’T:* - Test only the enhanced path - Skip backward compatibility tests - Assume feature flags work without testing - Test messages in isolation without context
...
❌ *Wrong - Reading environment in core:*
...
*Legacy tests failing:* - Ensure backward compatibility arities are preserved - Check that legacy callers don’t receive enhanced format unexpectedly - Verify feature flag defaults to `false`
...
;; Registry overview
(count (registry/get-all-rules))
(registry/get-rules-by-module :user)

;; Validate / Explain (Malli)
(m/validate user-schema/CreateUserRequest {:email "a@b.co" :name "A"})
(m/explain  user-schema/CreateUserRequest {:email "invalid"})

;; Coverage from tracked executions
...
* link:error-codes[Error Code Catalog] - Complete list of error codes and templates
* link:validation-patterns[Validation Patterns] - Message style guide and patterns
* ADR-005 - Architectural decision record
* User Module Implementation[User Module Implementation] - Concrete implementation examples

'''''

*For questions or feedback on validation features, see the link:https://github.com/thijs-creemers/boundary/blob/main/AGENTS[Developer Guide] or contact the development team.*
