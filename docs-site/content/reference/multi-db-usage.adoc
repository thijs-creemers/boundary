---
title: "Multi-Database Usage"
weight: 40
description: "Using multiple database drivers in a single Boundary system"
---
...
Legacy projects may still construct DB contexts manually. The factory-based approach is recommended:

[source,highlightjs,highlight]
----
;; OLD (legacy manual context construction)
(require '[boundary.platform.shell.adapters.database.common.connection :as conn]
         '[boundary.platform.shell.adapters.database.common.execution :as exec]
         '[boundary.platform.shell.adapters.database.sqlite.core :as sqlite])
(def datasource (conn/create-connection-pool config))
(def ctx {:datasource datasource
          :adapter (sqlite/new-adapter)})
(exec/execute-query! ctx query)
