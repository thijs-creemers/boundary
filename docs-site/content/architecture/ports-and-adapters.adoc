...
weight: 4
description: "Protocol-based dependency inversion and adapter patterns in Boundary"
---
...
----
;; SMTP email adapter
(ns boundary.platform.shell.email
  (:require [boundary.ports :as ports]
...
(ns boundary.platform.shell.system
  (:require [boundary.platform.shell.db :as db]
            [boundary.platform.shell.email :as email]
...
(ns boundary.platform.shell.adapters.caching
  (:require [boundary.ports :as ports]
...
1. **link:data-flow[Data Flow Architecture]** - How data flows through ports and adapters
2. **link:integration-patterns[Integration Patterns]** - Using ports and adapters in different interfaces
3. **link:configuration-and-env[Configuration and Environment Management]** - Wiring adapters based on configuration
