...
* Teams can work on different domains (user, user, inventory) without conflicts
...
(ns boundary.inventory.core.reservation
  (:require [boundary.user.ports :as user-ports]    ; ✓ Depend on user ports
            [boundary.inventory.core.stock :as stock]))

(defn create-reservation [user-repo item-repo request]
  ;; Use user module through its ports
  (let [user (user-ports/find-user-by-id user-repo (:user-id request))
        ;; Use inventory core logic
        reserved (stock/reserve-items item-repo (:items request))]
    {:reserved reserved
     :user-id (:id user)}))
----
