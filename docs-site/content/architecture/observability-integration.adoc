...
[abstract]
--
This guide provides practical instructions for integrating observability modules into Boundary Framework applications. It covers step-by-step setup, configuration, and usage patterns for logging, metrics, and error reporting in both development and production environments.
--
...
**Prerequisites:** Understanding of link:error-handling-observability.adoc[Error Handling & Observability] architecture
...
(ns boundary.platform.shell.interfaces.http.handlers.user
  (:require
...
(ns boundary.platform.shell.adapters.external.payment-service
  (:require
   [boundary.logging.core :as log]
   [boundary.metrics.core :as metrics]
   [boundary.error-reporting.core :as error]))
...
(ns boundary.platform.shell.utils.feature-flags
...
* **link:error-handling-observability.adoc[Error Handling & Observability]** - Architecture overview
* **Boundary Observability Integration** - `docs/guides/integrate-observability.adoc`
* **System Wiring Configuration** - `link:https://github.com/thijs-creemers/boundary/blob/main/src/boundary/shell/system/wiring.clj`
* **Protocol Implementations** - `link:https://github.com/thijs-creemers/boundary/blob/main/src/boundary/{logging,metrics,error-reporting}/shell/adapters.clj`