...
weight: 8
description: "Aero-based configuration, environment management, and secrets handling"
---
...
[abstract]
--
This document details configuration management strategies in Boundary Framework, covering environment-specific setup, secrets management, and system wiring patterns. It ensures the functional core remains pure while adapters receive appropriate configuration through dependency injection.
--
...
{:database {:host #env DATABASE_HOST
 ...
 :features {:metrics-collection true
           ...
           :structured-logging #or [#env BND_STRUCTURED_LOGGING false]
           :experimental-features false}}
----
...
(defn api-routes
  ...
  (let [routes []]
    (-> routes
        ...
        (with-module-routes :admin features boundary.modules.admin.http/routes)
        (with-module-routes :tenant features boundary.modules.tenant.http/routes)
...
1. **link:error-handling-observability[Error Handling and Observability]** - Monitoring configuration
