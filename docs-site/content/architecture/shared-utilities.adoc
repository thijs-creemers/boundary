...
weight: 11
description: "Type conversion, case transformation, and validation utilities"
---
...
[abstract]
--
This document provides comprehensive documentation for Boundary Framework's shared utility modules, which provide reusable type conversion, case transformation, and validation patterns across the application. These utilities were extracted from domain modules to promote code reuse, consistency, and maintainability.
--
...
**Prerequisites:** Understanding of link:overview[Architecture Overview] and basic Clojure concepts

== Key Takeaways

After reading this document, you will understand:

* ✅ **Shared utility architecture** and how utilities are organized
* ✅ **Type conversion patterns** for common data transformations
* ✅ **Case conversion utilities** for API and data transformations
* ✅ **Generic validation patterns** for schema-based validation
* ✅ **Integration patterns** for using shared utilities in domain modules
...
**🔄 Reusability**: Utilities can be used across multiple domain modules  
**🛡️ Nil-Safety**: All functions handle nil values gracefully
**📝 Domain-Agnostic**: Utilities contain no business logic, only technical transformations
**🧪 Comprehensive Testing**: All utilities have extensive test coverage

== Type Conversion Utilities

=== Purpose and Capabilities

The `boundary.core.utils.type-conversion` namespace provides generic type conversion functions that handle common data transformations needed across the application.

**Key Features:**
* UUID ↔ String conversions (nil-safe)
* Java Instant ↔ ISO-8601 String conversions
* Keyword ↔ String conversions
* Boolean ↔ Integer conversions (SQLite compatibility)
...
The `boundary.core.utils.case-conversion` namespace provides generic case transformation utilities for converting between different naming conventions commonly used in APIs and data structures.

**Key Features:**
* camelCase ↔ kebab-case map key transformations
...
The `boundary.core.utils.validation` namespace provides reusable validation patterns and utilities that work with Malli schemas across different contexts (API requests, CLI arguments, etc.).
...
* Standard case transformations (camelCase ↔ kebab-case)
* Generic validation patterns
* Nil-safe operations that are used across modules

**❌ Don't use shared utilities for:**
...
[source,clojure]
----
;; ✅ Good: Compose shared utilities with domain logic
...
;; ❌ Avoid: Modifying shared utilities for domain needs  
...
1. **link:components[Component Architecture]** - How utilities fit into the overall architecture
2. **link:data-flow[Data Flow Architecture]** - Validation and transformation pipelines  
