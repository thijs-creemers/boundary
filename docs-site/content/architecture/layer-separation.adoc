---
title: "Layer Separation Guidelines"
weight: 2
description: "Rules and examples for maintaining FC/IS architectural boundaries"
---
...
[WARNING]
====
**UPDATED ARCHITECTURE**: This document describes the original FC/IS patterns. For the current clean architecture implementation with domain-centric infrastructure, see **link:clean-architecture-layers[Clean Architecture Layer Separation]**.
...
**Prerequisites:** Understanding of link:overview[Architecture Overview] and functional programming principles
...
├── core/                  # Functional Core (pure domain logic)
├── ports.clj              # Abstract interfaces (protocols)
├── shell/                 # Imperative Shell (HTTP/CLI/DB adapters)
...
1. **link:components[Component Architecture]** - How to organize namespaces and modules
2. **link:ports-and-adapters[Ports and Adapters]** - Implementing dependency inversion
3. **link:data-flow[Data Flow Architecture]** - Request processing patterns
