...
weight: 3
description: "Domain-centric module structure with clean separation of business logic, services, and infrastructure"
---
...
[abstract]
--
This document establishes the updated layer separation guidelines for Boundary Framework following clean architecture principles. It provides concrete examples of the new domain-centric module structure with proper separation between business logic, application services, and infrastructure implementations.
--
...
**Prerequisites:** Understanding of link:overview[Architecture Overview] and link:layer-separation[Layer Separation Guidelines]
...
* ✅ **Development patterns** for building clean architectures
...
(defprotocol IUserRepository
  ...
  (find-user-by-email [this email]
    "Find user by email . Returns user entity or nil.")
  ...
  (find-users [this user-id options]
    "Find users with pagination. Returns {:users [] :total-count N}."))
...
(defrecord UserService [user-repository session-repository]
  ...
  (create-user [this user-data]
    ...
    (when (.find-user-by-email user-repository (:email user-data) (:user-id user-data))
...
(defn initialize-user-schema!
  ...
  (boundary.platform.shell.adapters.database.schema/initialize-tables-from-schemas! 
  ...
  (testing "creates and retrieves users from database"
    (let [ctx test-utils/*test-db-context*
      ...
      (let [created-user (.create-user repo user-data)
            retrieved-user (.find-user-by-email repo (:email user-data) (:user-id user-data))]
...
1. **link:overview[Architecture Overview]** - High-level architectural decisions
2. **link:components[Component Architecture]** - Detailed module implementation patterns
3. **link:ports-and-adapters[Ports and Adapters]** - Interface design patterns
