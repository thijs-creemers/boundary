...
....
flowchart LR
    subgraph mono["Day 1 — Modular Monolith"]
        direction TB
        U1["User Module"]
        O1["Order Module"]
        U1 <-->|in-process| O1
    ...
    mono -->|"Same code · no rewrites"| micro

    subgraph micro["Day 100 — Microservices"]
        direction TB
        U2["User Service"]
        O2["Order Service"]
        MB[("Message Bus")]
...
* link:../architecture/overview[Architecture Overview] — Deep dive into the FC/IS pattern
