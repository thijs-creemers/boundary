...
[arabic]
. link:#testing-philosophy[Testing Philosophy]
. link:#three-tier-strategy[Three-Tier Strategy]
. link:#unit-testing-core[Unit Testing (Core)]
. link:#integration-testing-shell-services[Integration Testing (Shell Services)]
. link:#contract-testing-httpadapters[Contract Testing (HTTP/Adapters)]
. link:++#test-organization--metadata++[Test Organization & Metadata]
. link:++#test-commands--workflow++[Test Commands & Workflow]
. link:#snapshot-testing[Snapshot Testing]
. link:++#test-fixtures--helpers++[Test Fixtures & Helpers]
. link:#accessibility-testing[Accessibility Testing]
. link:#testing-htmx-interactions[Testing HTMX Interactions]
. link:#mocking-strategies[Mocking Strategies]
. link:++#test-coverage--quality++[Test Coverage & Quality]
. link:#tdd-workflow[TDD Workflow]
...
[arabic]
. *Test the Logic, Mock the World*: Business logic resides in pure functions within the `core/` layer. These are tested with simple unit tests.
. *Isolate Side Effects*: All I/O, database access, and external communication happens in the `shell/` layer. We use integration and contract tests to verify these interactions.
. *Fast Feedback Loop*: Unit tests should be instantaneous. We prioritize them for complex logic.
. *Deterministic Results*: Tests must not depend on external state (except for a controlled test database).
. *No Property-Based Testing (Yet)*: Per current project roadmap, focus on example-based testing. Property-based testing is deferred.

==== The "`Test Pyramid`" in Boundary

We aim for a classic test pyramid: - *Unit (Top)*: Hundreds of tests, ultra-fast. - *Integration (Middle)*: Dozens of tests, verifying service wiring. - *Contract (Base)*: Critical path verification of external interfaces.
...
[width="99%",cols="15%,14%,21%,21%,29%",options="header",]
|===
|Tier |Focus |Location |Metadata |Dependencies
|*Unit* |Pure logic, transformations |`core/++*_++test.clj` |`:unit` |None
|*Integration* |Service orchestration |`shell/++*_++test.clj` |`:integration` |H2 DB / Mocked Ports
|*Contract* |HTTP/Persistence boundaries |`shell/++*_++test.clj` |`:contract` |Real DB (H2) {plus} HTTP
|===
...
libs/++{++library}/ ├── libs/++{++library}/src/boundary/++{++library}/ │ ├── core/ ++#++ Pure logic │ └── shell/ ++#++ I/O, adapters └── test/boundary/++{++library}/ ├── core/ ++#++ Unit tests └── shell/ ++#++ Integration & Contract tests
...
If a snapshot test fails after an intentional change: 1. Verify the diff in the test output. 2. If correct, run with `UPDATE++_++SNAPSHOTS=true`.
...
_Last updated: 2026-01-26_ _Documentation version: 1.0.0_

'''''

=== See also

* link:admin-testing.md[Admin Testing Guide] - Comprehensive admin UI testing strategies
* link:database-setup.md[Database Setup] - Database configuration for testing
* link:authentication.md[Authentication Guide] - Testing auth and MFA flows
* link:ide-setup.md[IDE Setup] - Configure your test runner in your IDE
