...
. link:#test-architecture[Test Architecture]
. link:#test-categories[Test Categories]
. link:#running-tests[Running Tests]
. link:#writing-new-tests[Writing New Tests]
. link:#testing-patterns[Testing Patterns]
. link:#accessibility-testing[Accessibility Testing]
. link:#coverage-goals[Coverage Goals]
...
*Current Coverage*: ~78% (lines), 90{plus} test cases
...
[width="100%",cols="21%,18%,24%,37%",options="header",]
|===
|Layer |Files |Purpose |Dependencies
|*Core* |`core/++*_++test.clj` |Pure business logic |None (pure functions)
|*Shell* |`shell/service++_++test.clj` |Service orchestration |H2 in-memory DB
|*Shell* |`shell/http++_++test.clj` |HTTP contracts |H2 {plus} minimal HTTP handler
|*Shell* |`shell/schema++_++repository++_++test.clj` |Config loading |H2 DB metadata
|===
...
*Location*: `test/boundary/admin/core/++*_++test.clj`

*Characteristics*: - ✅ Pure functions only (data in, data out) - ✅ No I/O, no database, no HTTP - ✅ Fast execution (++<++100ms per test) - ✅ Deterministic (same input → same output)
...
— unit (clojure.test) ————————— boundary.admin.core.ui-test render-field-value-test Render boolean values ✓ Render UUID values ✓ accessibility-aria-labels-test Icon buttons have aria-label ✓

27 tests, 232 assertions, 0 failures.

Top 3 slowest kaocha.type/var (0,02466 seconds, 28,3% of total time) boundary.admin.core.ui-test/htmx-attributes-test 0,01519 seconds boundary/admin/core/ui++_++test.clj:700
...
*Solution*: 1. Check function signature in source: `(defn create-service ++[++db-ctx config++]++ ...)` 2. Update test calls: `(create-service db-ctx ++{++:pagination ++{++...}})` 3. Search for all usages: `grep -r "create-service" test/`
...
* Test happy path {plus} edge cases (nil, empty, invalid)
* Test accessibility (labels, ARIA, semantic HTML)
...
* link:../testing.md[Boundary Testing Strategy] - Overall testing philosophy
* https://cljdoc.org/d/lambdaisland/kaocha/[Kaocha Documentation] - Test runner
* https://clojure.org/guides/testing[clojure.test Guide] - Built-in test framework
* https://www.w3.org/WAI/WCAG21/quickref/[WCAG 2.1 Guidelines] - Accessibility standards

'''''

*Questions?* See link:../../CONTRIBUTING.md[CONTRIBUTING.md] or ask in ++#++engineering Slack channel.
