...
* Inventory or ordering logic never mixes with user logic
* Workflow/state machines stay separate from user sessions
...
# Team B works on inventory module (no conflicts)
cd src/boundary/inventory
vim core/item.clj
----
...
Don't put inventory logic in the user module:

[source,clojure]
----
;; ❌ BAD: Inventory logic in user module
...
(defn create-reservation [user-id item-id]
  ...)  ; Only inventory concerns
----
...
├── inventory/            # Inventory operations only
└── orders/               # Order handling only
----
