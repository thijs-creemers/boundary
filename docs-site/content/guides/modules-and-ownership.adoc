---
title: "Modules and Domain Ownership"
weight: 4
description: "Organizing modules, domain boundaries, and team ownership"
---
...
A module is a self-contained package of functionality focused on a single business domain (like `user`, `storage`, or `jobs`). Each module contains:
...
* Storage or job processing logic never mixes with user logic
* Background processing stays separate from user sessions
...
cd libs/user/src/boundary/user
vim core/user.clj

# Team B works on storage module (no conflicts)
cd libs/storage/src/boundary/storage
vim core/storage.clj
----
...
[source,clojure]
----
;; inventory/core/stock.clj needs user data
(defn reserve-stock
  [user-repo reservation]  ; Accepts IUserRepository port
  (let [user (ports/find-user-by-id user-repo (:user-id reservation))]
    ;; perform reservation using user data
    ))
----
...
;; Storage module subscribes
;; Jobs module subscribes
(events/subscribe! :user/created
  (fn [event]
    (create-initial-credit (:user-id event))))
----
...
Not everything belongs in a domain module. Boundary provides `core` utilities for cross-cutting concerns:

[source]
----
libs/core/src/boundary/core/utils/
├── type_conversion.clj         # UUID, Instant conversions
├── case_conversion.clj         # camelCase ↔ kebab-case
└── validation.clj              # Generic validation utilities
----

**Use `core` utilities for:**
...
See link:create-module[Create a New Module] for step-by-step instructions.
...
Don't put storage or jobs logic in the user module:

[source,clojure]
----
;; ❌ BAD: Storage logic in user module
(ns boundary.user.core.user)
(defn create-user-with-inventory
  [user-data stock-item]  ; ❌ Inventory concerns don't belong here
...
(defn create-user [user-data]
  ...)  ; Only user concerns
;; In storage/jobs modules:
(ns boundary.storage.core.file)
(ns boundary.jobs.core.job)

(defn process-job [user-id item-id]
  ...)  ; Only module-specific concerns
----
...
* link:create-module[Create a New Module]
