...
[arabic]
. *Create* - Pure function creates job entity with default values
. *Enqueue* - Shell layer persists job to Redis queue
. *Dequeue* - Worker pulls job from queue (FIFO within priority)
. *Execute* - Handler function processes job arguments
. *Complete/Fail* - Result saved to Redis, job marked complete or queued for retry
...
*Why This Pattern Works*: - HTTP response returns immediately (++<++ 50ms) - Email sending happens asynchronously (1-5 seconds) - If email fails, automatic retry handles transient errors - User sees success page without waiting for email delivery
...
*Why This Pattern Works*: - Upload completes in ++<++ 1 second (just save original) - Image processing happens asynchronously (5-30 seconds depending on sizes) - Multiple sizes processed in parallel (if multiple workers) - Automatic retry handles transient failures (network issues, temporary disk full) - User sees immediate feedback, thumbnails appear when ready
...
*Why This Pattern Works*: - Reports generate at optimal time (low system load, users ready to read) - Scheduled jobs persist through restarts (stored in Redis) - Automatic retry if report generation fails - Users get consistent daily reports without manual intervention
...
*Why This Pattern Works*: - User request returns immediately with "`Export started`" message - Export runs at low priority, not affecting critical operations - Batch processing avoids memory issues with large datasets - Email notification when export completes - No retry on failure (user can request new export)
...
*Worker Count*: - *CPU-bound jobs* (image processing): Workers ≈ CPU cores - *I/O-bound jobs* (HTTP, database): Workers ≈ 2-4x CPU cores - *Mixed workload*: Start with 1-2 workers per core, tune based on metrics

*Redis Performance*: - *Single Redis instance*: 10,000{plus} jobs/second (simple jobs) - *Redis Cluster*: 50,000{plus} jobs/second (production workloads) - *Memory*: ~1 KB per queued job (including metadata)

*Latency Characteristics*: - *Enqueue*: ++<++ 5ms (Redis write {plus} network) - *Dequeue*: ++<++ 10ms (Redis read {plus} deserialization) - *Processing*: Depends on job handler (see table above) - *Total latency*: Enqueue {plus} Queue time {plus} Processing {plus} Result storage
...
*Common causes*: - External service down (API, database) - Invalid job arguments (schema validation) - Timeout (increase job timeout) - Resource exhaustion (memory, file handles)
...
* *link:{{< relref "caching.md" >}}[Distributed Caching Guide]* - Cache frequently accessed data
* *link:{{< relref "integrate-observability.adoc" >}}[Observability Guide]* - Monitor job processing metrics
* *link:../../README.md[Module Development]* - Integrate jobs into your modules
* *https://github.com/thijs-creemers/boundary[GitHub Repository]* - View source code and examples
