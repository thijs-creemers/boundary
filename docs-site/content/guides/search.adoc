...
* *Relevance Ranking*: Automatic `ts++_++rank` scoring with field weighting (name ++>++ email ++>++ bio)
...
[width="100%",cols="18%,26%,39%,17%",options="header",]
|===
...
|*Shell* |`postgresql.clj` |SQL generation with `plainto++_++tsquery`, `ts++_++rank` |❌ No
...
[arabic]
. *Request*: User submits query "`software engineer`" via `/api/search/users?q=software{plus}engineer`
. *Parsing*: Service parses query into internal DSL (match query)
. *SQL Generation*: Provider builds `WHERE search++_++vector @@ plainto++_++tsquery('software engineer')`
. *Database Query*: PostgreSQL executes full-text search using GIN index
. *Ranking*: Core calculates base scores (ts++_++rank) and applies recency boost
. *Highlighting*: Core wraps matching terms in `++<++mark++>++` tags
. *Response*: JSON response with ranked, highlighted results
...
*Why This Pattern Works*: - *Comprehensive matching*: Single query searches name, email, and bio fields - *Relevant results*: PostgreSQL `ts++_++rank` with field weighting (name ++>++ email ++>++ bio) - *Visual feedback*: Highlighted terms show users exactly what matched - *Performance*: GIN index keeps queries ++<++ 50ms for typical datasets
...
*Why This Pattern Works*: - *Fast feedback*: ++<++ 20ms response times for prefix queries - *Progressive refinement*: Suggestions narrow as user types more characters - *Reduced typos*: Users select from validated suggestions - *Better UX*: Instant feedback improves perceived performance
...
*Why This Pattern Works*: - *Multi-field matching*: Single query searches name, SKU, and location - *Faceted filtering*: Combine full-text search with exact filters (category, stock status) - *Field weighting*: SKU matches rank higher than location matches - *Business logic*: Disable recency boost for inventory (time-independent relevance)
...
|With filters |"`laptop {plus} Warehouse A`" |20-40ms
...
*Why This Pattern Works*: - *Fresh content prioritized*: Recent posts rank higher even with lower text relevance - *Graceful decay*: Boost decreases linearly over configured period (30 days default) - *Configurable*: Adjust `max-boost` and `decay-days` per use case - *Transparent*: Final scores include boost, visible in response
...
Search requires a `search++_++vector` column with GIN index:
...
*Weight Meanings*: - `'A'` (1.0): Highest weight - names, titles - `'B'` (0.4): Medium-high weight - emails, summaries +
- `'C'` (0.2): Medium weight - descriptions, content - `'D'` (0.1): Lowest weight - metadata, tags
...
|Highlight |Any |{plus}2-5ms |{plus}8ms |++<++ 10ms |✅ Good
|Recency boost |Any |{plus}1-3ms |{plus}5ms |++<++ 5ms |✅ Excellent
|===

=== Query Performance by Complexity

[width="100%",cols="39%,28%,18%,15%",options="header",]
|===
|Query Type |Example |Time |Why
|*Single term* |"`john`" |10-20ms |Fastest, single token lookup
|*Multi-term* |"`software engineer`" |15-30ms |Multiple token lookups {plus} AND
...
*Symptoms*: `++_++highlights` field missing or doesn’t contain `++<++mark++>++` tags.
...
* *link:pagination.md[API Pagination]*: Implement cursor-based pagination for large search result sets
* *link:caching.md[Distributed Caching]*: Cache frequent search queries with Redis
* *link:background-jobs.md[Background Jobs]*: Build search indexes asynchronously
* *link:../reference/metrics.md[Performance Monitoring]*: Track search latency and throughput
...
* [ ] *Query timeouts*: Set `statement++_++timeout` in PostgreSQL (10-30 seconds)
...
[width="100%",cols="33%,31%,18%,18%",options="header",]
|===
...
Full API documentation: link:../../docs/SEARCH_API_REFERENCE.md[SEARCH++_++API++_++REFERENCE.md]
