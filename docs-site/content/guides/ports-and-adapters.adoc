...
weight: 3
description: "Using protocol-based dependency inversion in Boundary"
---
...
graph TB
    subgraph "External Interfaces (Primary Adapters)"
        HTTP[🌐 HTTP Adapter<br/>Ring/Reitit]
        CLI[🖥️ CLI Adapter<br/>tools.cli]
        WEB[📱 Web Adapter<br/>HTMX]
    end
    ...
    subgraph "Functional Core (Hexagon)"
        direction TB
        CORE[💎 User Core<br/>Pure Business Logic<br/><br/>• prepare-user-creation<br/>• validate-user-data<br/>• calculate-user-tier<br/>• enrich-user-profile]
    end
    ...
    subgraph "External Systems (Secondary Adapters)"
        DB[🗄️ Database Adapter<br/>PostgreSQL]
        SMTP[📧 Email Adapter<br/>SMTP/SendGrid]
        LOGGER[📝 Audit Adapter<br/>Database Logger]
    end
...
* ✅ **Core Independence**: Core logic doesn't depend on any specific technology
* ✅ **Multiple Interfaces**: Same core logic serves HTTP, CLI, and Web without changes
* ✅ **Easy Testing**: Mock implementations of ports for testing core logic
* ✅ **Flexible Infrastructure**: Swap database from PostgreSQL → H2 → SQLite without touching core
...
* link:implement-port-adapter[Implement a Port and Adapter]
