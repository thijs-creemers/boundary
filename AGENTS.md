...
| **Placeholder tests** | `bb check:placeholder-tests` | `(is true)`-style shapes, a deftest with no assertion at all (helpers like `check-snapshot!` count via a named list), and identical-token `(= x x)` tautologies in `is`/`are`. Escape hatch: `^:wagoe/allow-placeholder` on the deftest | Yes |
