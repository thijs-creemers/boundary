...
(defn- add-table! [^XWPFDocument doc columns data]
  (let [n-cols (count columns)
    ...
    (doseq [[row-idx record] (map-indexed vector data)]
      (doseq [[col-idx value] (map-indexed vector (core/map-columns columns record))]
        (-> (.getRow table (inc row-idx))
            (.getCell col-idx)
            (.setText (str value)))))
    table))

(defn- render-section! [^XWPFDocument doc {:keys [type content columns]} data]
  (case type
    :header    (add-heading!    doc content)
    :paragraph (add-paragraph!  doc content)
    :footer    (add-footer-para! doc content)
    :table     (add-table!      doc columns data)
  ...
  (generate! [_this report-def data _opts]
    ...
    (let [doc      (XWPFDocument.)
      ...
      (doseq [section sections]
        (render-section! doc section data))
