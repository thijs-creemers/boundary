(ns boundary.platform.shell.adapters.database.h2.utils
...
(defn create-test-context
  ...
  ([db-name]
   (let [adapter   ((requiring-resolve 'boundary.platform.shell.adapters.database.h2.core/new-adapter))
         db-config {:adapter       :h2
                    :database-path (str "mem:" db-name)}]
     {:adapter    adapter
      :datasource ((requiring-resolve 'boundary.platform.shell.adapters.database.common.core/create-connection-pool)
                   adapter db-config)})))
