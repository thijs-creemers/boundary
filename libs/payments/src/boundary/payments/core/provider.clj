...
(defn stripe-intent-status->payment-status
  "Map a Stripe PaymentIntent status string to a PaymentStatus keyword.
   ...
   requires_action depends on context: in the default (poll) context the
   customer may still complete authentication, so it maps to :pending.
   With {:off-session? true} it maps to :failed — an off-session charge that
   needs SCA cannot be completed unattended; bring the customer on-session."
...
(defn stripe-checkout-params
  ...
  (cond-> {"line_items[0][price_data][currency]"           (str/lower-case (or currency "eur"))
    ...
    setup-future-usage
    (assoc "payment_intent_data[setup_future_usage]"
           (case setup-future-usage
             :off-session "off_session"
             :on-session  "on_session"))

    (and setup-future-usage (nil? provider-customer-id))
