...
`boundary new` or hand-wired against the published `org.boundary-app/*`
...
Platform's `boundary.platform.shell.system.wiring` no longer requires the
...
(:require [boundary.platform.shell.system.wiring]
          [boundary.user.shell.module-wiring]        ; always (user is core)
          ;; per enabled module:
...
Projects generated by `boundary new` on alpha-42+ get this automatically
...
|---|---|
| `boundary.platform.shell.interfaces.http.tenant-middleware` | `boundary.tenant.shell.tenant-middleware` |
| `boundary.platform.shell.interfaces.http.membership-middleware` | `boundary.tenant.shell.membership-middleware` |
...
`boundary.shared.ui.*` namespaces moved from the admin lib into a new
`org.boundary-app/boundary-shared-ui` artifact. The namespaces themselves are
...
`org.boundary-app/boundary-shared-ui` or — better — trim the list to the
