...
System wiring (`boundary.platform.shell.system.wiring`) composes routes:
...
1. **libs/user/src/boundary/user/shell/http.clj**
...
4. **libs/platform/src/boundary/platform/shell/interfaces/http/routes.clj**
...
4. **Deprecation Cleanup**
   ...
   - Clean up `boundary.platform.shell.modules` compose-http-handlers
...
[source,clojure]
----
;; 1. Define routes in boundary.inventory.shell.http
...
;; 2. Wire in boundary.inventory.shell.module-wiring
