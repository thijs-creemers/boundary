...
The Boundary Framework currently supports single-tenant deployments
...
with Boundary's existing modules (user, admin, jobs, cache, realtime).
...
. *Scales Well*: Handles 100-10,000 tenants efficiently (Boundary's
target)
...
* Located:
`libs/platform/src/boundary/platform/shell/interfaces/http/tenant++_++middleware.clj`
* Tests:
`libs/platform/test/boundary/platform/shell/interfaces/http/tenant++_++middleware++_++test.clj`
...
* link:../research/multi-tenancy-patterns.md[Multi-Tenancy Patterns
Research] - Comprehensive analysis of all patterns
...
. Implement `boundary.platform.core.tenant` namespace (pure functions)
...
* `libs/tenant/test/boundary/tenant/shell/provisioning++_++test.clj`
(250{plus} lines)
...
* `libs/tenant/src/boundary/tenant/shell/http.clj` - Added provisioning
endpoints
* `libs/tenant/src/boundary/tenant/shell/module++_++wiring.clj` -
Integrant wiring
...
* `libs/jobs/src/boundary/jobs/shell/tenant++_++context.clj` (280 lines)
...
* `libs/jobs/test/boundary/jobs/shell/tenant++_++context++_++test.clj`
(330{plus} lines)
...
* `libs/tenant/src/boundary/tenant/shell/provisioning.clj` ({plus}62
lines)
...
* `libs/cache/src/boundary/cache/shell/tenant++_++cache.clj` (370{plus}
lines)
...
namespaces)
* `libs/cache/test/boundary/cache/shell/tenant++_++cache++_++test.clj`
(440{plus} lines)
...
* `libs/tenant/test/boundary/tenant/integration++_++test.clj` (730{plus}
lines, 7 scenarios)
...
* `libs/tenant/test/boundary/tenant/integration++_++test.clj` ({plus}15
lines)
