...
|*ui-style* |`org.boundary-app/boundary-ui-style` |None
...
Generate a deploy token: 1. Go to https://clojars.org/tokens[Clojars Dashboard] 2. Click "`Generate Token`" 3. Name it (e.g., "`boundary-deploy`") 4. Copy the token (shown only once)
...
7. **ui-style** (no runtime deps, but should be published before admin)
8. **admin** (depends on platform, user, ui-style)
...
LIBS_ORDER="core observability platform scaffolder user storage ui-style admin"
...
For issues with publishing: - Check https://github.com/clojars/clojars-web/wiki[Clojars documentation] - Open an issue on the Boundary repository

'''''

=== See also

* link:../guides/security-setup.md[Security Setup] - Repository security and Clojars access control
* link:../guides/operations.adoc[Operations Guide] - Production deployment after publishing
* link:../guides/testing.md[Testing Guide] - Test before publishing
