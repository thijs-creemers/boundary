...
|`defworkflow` macro, guards, side effects via jobs, lifecycle hooks, auto-transitions, audit trail, REST API

|`libs/search`
|Full-text search
|Index management, query building, search result UI components, admin integration

|`libs/reports`
|Report generation and export
|`defreport` macro, PDF export (OpenHTMLToPDF), CSV/Excel export (Docjure), scheduler integration

|`libs/calendar`
|Calendar and scheduling
|`defevent` macro, RRULE recurrence (ical4j), iCal import/export, conflict detection, Hiccup calendar views

|`libs/geo`
|Geocoding and geospatial utilities
|Multi-provider geocoding (OSM / Google / Mapbox), DB-backed cache, Haversine distance calculation
...
|Compiled CSS/JS assets, design tokens, `:base`, `:pilot`, `:admin-pilot` bundles (Tailwind + DaisyUI)
|===
...
# Standalone libraries (no internal deps)
reports   # uses openhtmltopdf + docjure directly
calendar  # uses ical4j directly
geo       # uses clj-http + next.jdbc directly
ai        # uses clj-http + integrant directly
ui-style  # CSS/JS assets only
----
...
* `workflow` and `search` integrate with the admin UI and therefore depend on `admin`
* `reports`, `calendar`, `geo`, `ai`, and `ui-style` are standalone — no internal library dependencies
...
* link:publishing.adoc[Publishing Guide] - Building and deploying all 20 libraries to Clojars
