...
|`boundary-tools/`
|Babashka developer tooling (not under `libs/`)
...
├── boundary-tools/              # Babashka developer tooling (bb tasks)
│   ├── src/boundary/tools/      # scaffold, ai, doctor, setup, deploy, etc.
│   └── test/boundary/tools/     # Unit tests (bb test:tools)
├── resources/
...
* `payments`, `i18n`, and `ui-style` are standalone — no internal Boundary deps
* `boundary-tools` is consumed via `{:local/root "boundary-tools"}` in `bb.edn`, not as a library dependency
