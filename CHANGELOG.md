...
- **`boundary-cli`**: `boundary agents update` (+ generated `bb agents:update` task) refreshes the framework-owned sections of a project's `AGENTS.md` after a Boundary upgrade. The marker-delimited blocks (`gen:fc-is`, `gen:naming`, `gen:pitfalls`, `boundary:available-modules`) are re-rendered from the installed CLI's template and spliced in place; everything outside the markers — team notes, custom sections — is left untouched, the `installed-modules` block is treated as project state, and installed modules stay removed from the refreshed available table (mirroring `boundary add`). `--check` exits 1 when the file is stale, for CI. Idempotent.
