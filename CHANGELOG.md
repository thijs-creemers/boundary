...
- **`boundary-jobs`**: Multi-instance hardening (BOU-88). (1) A dequeued job whose type has no handler on the local worker is now **re-enqueued** for another instance (bounded by `:max-requeues`, default 5, tracked in job metadata) instead of being silently dead-lettered; it is failed terminally with a `NoHandlerError` only once the budget is exhausted (no instance handles it). The re-enqueue is *delayed* (parked in the scheduled set `:requeue-delay-ms` ahead, default 1000) so a handlerless worker cannot reacquire the job on its next immediate poll and burn the whole budget in a tight loop before a handler-owning worker polls. A worker created with an empty handler registry logs a loud warning at startup. (2) Scheduled-job promotion is now an **atomic claim** so concurrent workers can't both move (and run) the same due job: the Redis adapter uses `ZREM` as the claim (only the worker whose `ZREM` returns 1 enqueues) and the in-memory adapter uses `swap-vals!`. Both `process-scheduled-jobs!` implementations now return the count actually promoted.
