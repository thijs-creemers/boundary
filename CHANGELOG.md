...
- **Dependencies brought up to date** (BOU-443). 31 coordinates across 34 `deps.edn`,
...
- **The Render and Fly blueprints deployed a container that served nothing** (BOU-439).
  Both now build the root `Dockerfile` and set `ERROR_REPORTING_PROVIDER=no-op`.
...
- **`bb setup` dropped the dev error enricher `wagoe new` configured** (BOU-416). If
  setup rewrote your config, add `:wagoe/dev-error-enricher {}` under dev `:active`.
...
- **The stability page said the wrong current version** (BOU-413). Its current-version
  cell is now gated by `check:versions` instead of exempted with the rest of the page.
- **The documented `bbin install` pinned a tag that does not exist** (BOU-412). Our
  tags carry no `v` prefix; `cli.adoc` now says so and the gate requires the bare form.
