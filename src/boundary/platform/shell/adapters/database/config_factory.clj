(ns boundary.platform.shell.adapters.database.config-factory
  "Configuration-driven database adapter factory.
   ...
   Usage:
     (require '[boundary.platform.shell.adapters.database.config-factory :as cf])
  ...
  (:require [boundary.platform.shell.adapters.database.config :as db-config]
            [boundary.platform.shell.adapters.database.common.core :as core]
  ...
  (if (db-config/adapter-active? adapter-type env)
    (do
      ...
      (let [adapter-ns (case adapter-type
                         :sqlite 'boundary.platform.shell.adapters.database.sqlite.core
                         :postgresql 'boundary.platform.shell.adapters.database.postgresql.core
                         :mysql 'boundary.platform.shell.adapters.database.mysql.core
                         :h2 'boundary.platform.shell.adapters.database.h2.core)
