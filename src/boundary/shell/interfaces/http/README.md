...
The `boundary.shell.interfaces.http` module provides a comprehensive HTTP interface infrastructure for the Boundary application. It implements a clean, modular architecture following clean architecture principles and provides reusable components for building REST APIs.
...
```
boundary.shell.interfaces.http/
...
```clojure
(require '[boundary.shell.interfaces.http.middleware :as middleware])
